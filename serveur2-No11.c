      ...
      while (ack_max != packets_number){
        ...
        if (select_value == -1)
          ...
          *shared_memory_seq=ack_max+1; //retransmission à partir du ACK max reçu
          ...
          timeout.tv_usec = 20*srtt.tv_usec; //on sécurise le temps d'attente de retransmission car il y a congestion (inférieure à serveur1 pour détecter plus de timeout)
