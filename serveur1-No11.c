...
int main(int argc, char* argv[]){
  ...
  while(1){
    ...
    int window=40; //cette valeur va servir de seuil pour fixer le nombre de segment qu'on envoit
    int ack_fin = 0;
    ...
    while (ack_fin != packets_number+1){ //phase envoie de segments
      ...
      printf("SLEEP\n");
      sleep(timeout.tv_usec*pow(10,-5)); //le temps de recevoir les derniers acks envoyés par le client
